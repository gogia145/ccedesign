<div class="newsletters <?php echo $this -> pre; ?>unsubscribe <?php echo $this -> pre; ?>">
	...
	<?php if (!empty($success) && $success == true) : ?>
		...
		<?php if (empty($deleted) && $deleted == false) : ?>
			<ul>
				<li><?php _e('Go back to', 'wp-mailinglist'); ?> <a href="<?php echo home_url(); ?>" title="<?php echo esc_attr(stripslashes(get_bloginfo('name'))); ?>"><?php echo get_bloginfo('name'); ?></a></li>
			</ul>
		<?php endif; ?>
	<?php elseif (!empty($data)) : ?>
		...
		<form action="<?php echo $_SERVER['REQUEST_URI']; ?>" method="post">
			...
			<p class="submit">
				<button type="submit" name="confirm" value="1" class="<?php echo $this -> pre; ?>button">
