	...
	<tbody>
		<tr>
			...
			<td>
				    	...
				    	<div id="languagetabsconfirm">
				            ...
				            <?php foreach ($languages as $language) : ?>
				            	<div id="languagetabconfirm<?php echo $tabnumber; ?>">
				            		<input type="text" placeholder="<?php echo esc_attr(stripslashes($this -> et_subject('confirm', null, $language))); ?>" name="etsubject_confirm[<?php echo $language; ?>]" value="<?php echo esc_attr(stripslashes($this -> language_use($language, $etsubject_confirm))); ?>" id="etsubject_confirm_<?php echo $language; ?>" class="widefat" />
				            	</div>
				            	<?php $tabnumber++; ?>
				            <?php endforeach; ?>
				    	</div>
				...
				<?php else : ?>
					<input type="text" placeholder="<?php echo esc_attr(stripslashes($this -> et_subject('confirm'))); ?>" name="etsubject_confirm" value="<?php echo esc_attr(stripslashes($this -> get_option('etsubject_confirm_form_' . $form -> id))); ?>" id="etsubject_confirm" class="widefat" />
				<?php endif; ?>
			</td>
		</tr>
		<tr>
			...
			<td>
				...
				<?php else : ?>
					...
					wp_editor(stripslashes($this -> get_option('etmessage_confirm_form_' . $form -> id)), 'etmessage_confirm', $settings); 
