<div class="wrap" style="width:450px;">
	...
	<h2><?php _e('Step 3: Verify the DKIM setup', 'wp-mailinglist'); ?></h2>
	
	<p>
		<?php _e('All done! Save the settings then send an email using the Test Email Settings utility to the email address below and you will receive the results on your From Address in a few minutes.', 'wp-mailinglist'); ?>
	</p>
	...
	<span style="display:none;">	
		<form action="" onsubmit="jQuery('#dkimbutton').prop('disabled', true); jQuery('#dkimloading').show(); dkimwizard(jQuery(this).serialize()); return false;" id="dkimform3">
			...
			<button value="1" type="submit" name="continue">
				<?php _e('Continue', 'wp-mailinglist'); ?>
			</button>
		</form>
	</span>
	...
	<p class="submit">
		...
		<input onclick="jQuery('#goto').val('step2'); jQuery('#dkimform3').submit();" type="button" class="button button-secondary" name="back" value="<?php _e('&laquo; Back', 'wp-mailinglist'); ?>" />
		<input id="dkimbutton" onclick="jQuery('#settings-form').submit();" type="button" class="button button-primary" name="continue" value="<?php _e('Finished, save the settings &raquo;', 'wp-mailinglist'); ?>" />
