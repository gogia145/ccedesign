...
<table class="form-table">
	<tbody>
		<tr>
			<th><label for="etsubject_order"><?php _e('Email Subject', 'wp-mailinglist'); ?></label></th>
			<td>
				    	...
				    	<div id="languagetabsorder">
				            ...
				            <?php foreach ($languages as $language) : ?>
				            	<div id="languagetaborder<?php echo $tabnumber; ?>">
				            		<input type="text" name="etsubject_order[<?php echo $language; ?>]" value="<?php echo esc_attr(stripslashes($this -> language_use($language, $texts))); ?>" id="etsubject_order_<?php echo $language; ?>" class="widefat" />
				            	</div>
				            	<?php $tabnumber++; ?>
				            <?php endforeach; ?>
				    	</div>
		...
		<tr>
			<th><label for="etmessage_order"><?php _e('Email Message', 'wp-mailinglist'); ?></label></th>
			<td>
				<?php if ($this -> language_do()) : ?>
					<?php if (!empty($languages) && is_array($languages)) : ?>
		...
		<tr>
			<th><label for="ettemplate_order"><?php _e('Email Template', 'wp-mailinglist'); ?></label></th>
			<td>
				<?php $ettemplate_order = __($this -> get_option('ettemplate_order')); ?>
				<?php if ($themes = $Theme -> select()) : ?>
					<select name="ettemplate_order" id="ettemplate_order">
						...
						<?php foreach ($themes as $theme_id => $theme_title) : ?>
							<option <?php echo (!empty($ettemplate_order) && $ettemplate_order == $theme_id) ? 'selected="selected"' : ''; ?> value="<?php echo $theme_id; ?>"><?php _e($theme_title); ?></option>
						<?php endforeach; ?>
					</select>
				<?php else : ?>
