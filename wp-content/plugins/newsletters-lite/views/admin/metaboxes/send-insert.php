...
<div id="inserttabs">
	<ul>
		<?php if (!empty($inserttabs['fields'])) : ?><li><a href="#inserttabs-1"><?php _e('Fields', 'wp-mailinglist'); ?></a></li><?php endif; ?>
		<?php if (!empty($inserttabs['posts'])) : ?><li><a href="#inserttabs-2"><?php _e('Posts', 'wp-mailinglist'); ?></a></li><?php endif; ?>
	...
	<?php if (!empty($inserttabs['fields'])) : ?>
		<div id="inserttabs-1">
	        ...
	        <?php if (!empty($Subscriber -> table_fields)) : ?>
	        	<p>
		        	<a href="" onclick="jQuery('#morefieldslist').toggle(); return false;" class="button button-primary"><i class="fa fa-caret-down"></i> <?php _e('More Fields', 'wp-mailinglist'); ?></a>
	        	</p>
	...
	<?php if (!empty($inserttabs['posts'])) : ?>
		<div id="inserttabs-2">
			<h4><?php _e('Insert Posts', 'wp-mailinglist'); ?> <?php echo $Html -> help(__('Insert single posts, multiple posts and post featured images into your newsletter as needed. Follow the selections below to make the posts available and then click to insert.', 'wp-mailinglist')); ?></h4>
		
			<p>
				<label><input type="radio" name="ptype" checked="checked" value="single" id="ptype_single" /> <?php _e('Single', 'wp-mailinglist'); ?></label>
				<label><input type="radio" name="ptype" value="page" id="ptype_page" /> <?php _e('Page', 'wp-mailinglist'); ?></label>
				<br/>
				<label><input type="radio" name="ptype" value="multiple" id="ptype_multiple" /> <?php _e('Multiple', 'wp-mailinglist'); ?></label>
				<label><input type="radio" name="ptype" value="thumbnail" id="ptype_thumbnail" /> <?php _e('Thumbnail', 'wp-mailinglist'); ?></label>
			</p>
			
			<div id="ptypeglobal" style="display:block;">
				<?php if ($this -> language_do()) : ?>
					<label for=""><?php _e('Language:', 'wp-mailinglist'); ?></label>
		        ...
		        <p>
					<label for="post_showdate_Y"><?php _e('Show Date:', 'wp-mailinglist'); ?></label>
					<label><input type="radio" name="post_showdate" value="Y" id="post_showdate_Y" checked="checked" /> <?php _e('Yes', 'wp-mailinglist'); ?></label>
				...
				<p>
					<label for="post_eftype_excerpt"><?php _e('Display:', 'wp-mailinglist'); ?></label>
					<label><input type="radio" name="post_eftype" value="full" id="post_eftype_full" /> <?php _e('Full', 'wp-mailinglist'); ?></label>
					<label><input type="radio" name="post_eftype" value="excerpt" id="post_eftype_excerpt" checked="checked" /> <?php _e('Excerpt/Short', 'wp-mailinglist'); ?></label>
			...
			<div id="ptypediv_single" style="display:block;">
				<?php if ($posttypes = $this -> get_custom_post_types(true)) : ?>
					<p>
						<label for="posttype"><?php _e('Post Type:', 'wp-mailinglist'); ?></label><br/>
						<select style="max-width:200px;" onchange="change_posttype(this.value)" name="posttype" id="posttype">
							<option value="post"><?php _e('Post', 'wp-mailinglist'); ?></option>
				...
				<div id="posttype_post" style="display:block">
					<p>
						<label for="posts_category_menu"><?php _e('Post Category:', 'wp-mailinglist'); ?></label><br/>
				...
				<div id="postsdiv" style="display:none;">
					<p>
						<label for=""><?php _e('Choose Post:', 'wp-mailinglist'); ?></label>
						...
						<div id="ajaxposts">
							<span class="howto"><?php _e('Choose all settings above.', 'wp-mailinglist'); ?></span>
						</div>
					</p>
				</div>
			</div>
			...
			<div id="ptypediv_page" style="display:none;">
				<p>
					<label for="page_id"><?php _e('Page:', 'wp-mailinglist'); ?></label>
					<?php wp_dropdown_pages(array('depth' => 0, 'child_of' => 0, 'echo' => 1, 'name' => "page_id", 'show_option_none' => false)); ?>
				</p>
				
				<input type="button" class="button button-secondary" onclick="insert_post(jQuery('#page_id').val(), false);" name="insertpage" value="<?php _e('Insert Page', 'wp-mailinglist'); ?>" />
			</div>
			
			<div id="ptypediv_multiple" style="display:none;">
				<p>
					<label for=""><?php _e('Number:', 'wp-mailinglist'); ?></label>
				...
				<p>
					<label for=""><?php _e('Order:', 'wp-mailinglist'); ?></label><br/>
	                ...
	                <select name="posts_order" id="posts_order" style="max-width:100px;">
	                	<option value="ASC"><?php _e('Ascending', 'wp-mailinglist'); ?></option>
	                	<option value="DESC"><?php _e('Descending', 'wp-mailinglist'); ?></option>
	                </select>
	                <?php echo $Html -> help(__('Order', 'wp-mailinglist')); ?>
				</p>
				
				<p>
					<label for=""><?php _e('Posts Category:', 'wp-mailinglist'); ?></label><br/>
				...
				<p>
					<label for=""><?php _e('Post Types:', 'wp-mailinglist'); ?></label>
				...
				<input onclick="insert_post(false, false);" type="button" name="insertmultiple" class="button button-secondary" value="<?php _e('Insert Posts', 'wp-mailinglist'); ?>" />
			</div>
			...
			<div id="ptypediv_thumbnail" style="display:none;">
				...
				<p>
					<label for="thumbnail_size"><?php _e('Thumbnail Size:', 'wp-mailinglist'); ?></label><br/>
				...
				<input type="button" class="button button-secondary" onclick="insert_post(false, false);" name="insertthumbnail" value="<?php _e('Insert Thumbnail', 'wp-mailinglist'); ?>" />
			</div>
				...
				<ul class="insertfieldslist">
					<?php foreach ($snippets as $snippet) : ?>
						<li>
							...
							<?php if (apply_filters($this -> pre . '_admin_createnewsletter_loadintoeditorlinks', true)) : ?><small><a href="?page=<?php echo $this -> sections -> send; ?>&method=snippet&id=<?php echo $snippet -> id; ?>" class=""><?php _e('Load into Editor', 'wp-mailinglist'); ?></a></small><?php endif; ?>
						</li>
					<?php endforeach; ?>
				</ul>
			<?php else : ?>
				<p class="newsletters_error"><?php _e('No email snippets available.', 'wp-mailinglist'); ?></p>
			<?php endif; ?>
		</div>
	<?php endif; ?>
</div>
