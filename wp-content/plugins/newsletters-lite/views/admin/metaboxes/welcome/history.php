<?php if (!empty($histories)) : ?>
			...
			<?php foreach ($histories as $history) : ?>
				<tr class="<?php echo $class = (empty($class)) ? 'alternate' : ''; ?>">
					<td>
						...
						<h3>
							<p class="submit">
								<a class="button button-secondary" href="<?php echo admin_url('admin.php?page=' . $this -> sections -> history . '&method=view&id=' . $history -> id); ?>"><i class="fa fa-eye"></i> <?php _e('View', 'wp-mailinglist'); ?></a>
								<a class="button button-primary" href="<?php echo admin_url('admin.php?page=' . $this -> sections -> send . '&amp;method=history&amp;id=' . $history -> id); ?>"><i class="fa fa-paper-plane"></i> <?php _e('Send/Edit', 'wp-mailinglist'); ?></a>
							</p>
							<a href="<?php echo admin_url('admin.php?page=' . $this -> sections -> history . '&method=view&id=' . $history -> id); ?>"><?php _e($history -> subject); ?></a>
							<p class="howto">
								<?php echo sprintf(__('Created on %s by %s', 'wp-mailinglist'), $Html -> gen_date("M j, Y", strtotime($history -> created)), ((!empty($history -> user_id)) ? get_the_author_meta('display_name', $history -> user_id) : __('unknown', 'wp-mailinglist'))); ?>
							</p>
						</h3>
						...
						<p>
							...
							?>
							<a href="?page=<?php echo $this -> sections -> history; ?>&amp;method=view&amp;id=<?php echo $history -> id; ?>"><?php echo sprintf("%s / %s / %s / %s", '<span style="color:#46BFBD;">' . number_format($tracking, 2, '.', '') . '&#37;</span>', '<span style="color:#FDB45C;">' . number_format($eunsubscribeperc, 2, '.', '') . '&#37;</span>', '<span style="color:#F7464A;">' . number_format($ebouncedperc, 2, '.', '') . '&#37;</span>', $clicks); ?></a>
	...
	<p class="textright">
		...
		<?php _e('or', 'wp-mailinglist'); ?> <a href="<?php echo admin_url('admin.php?page=' . $this -> sections -> send); ?>"><?php _e('create a new one', 'wp-mailinglist'); ?></a>
	</p>
<?php else : ?>
	<p>
		<?php _e('Sent emails and saved drafts will be displayed here as soon as you create them.', 'wp-mailinglist'); ?>
	</p>
<?php endif; ?>