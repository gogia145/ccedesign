...
class wpmlQueue extends wpMailPlugin {
	...
	function save($subscriber = null, $user = null, $subject = null, $message = null, $attachments = array(), $post_id = 0, $history_id = 0, $return_query = false, $theme_id = 0, $senddate = null) {
		...
		if (empty($senddate) || strtotime($senddate) < current_time('timestamp')) {
			$this -> set_timezone();
			$senddate = $Html -> gen_date("Y-m-d H:i:s");
		}
		
		if (empty($subscriber) && empty($user)) { $this -> errors[] = __('No subscriber specified', 'wp-mailinglist'); }		
		if (empty($subject)) { $this -> errors[] = __('No subject specified', 'wp-mailinglist'); }
		else { $slug = $Html -> sanitize($subject); }		
		if (empty($message)) { $this -> errors[] = __('No message specified', 'wp-mailinglist'); }
			...
			if (!empty($subscriber)) {
				...
				if (empty($return_query) || !$return_query) {
					if ($wpdb -> query($query)) {
						...
						$this -> errors[] = __('Query failed, email may already be queued?', 'wp-mailinglist');
					}
				} else {
					return $query;	
				}
